...
features = features.reshape((len(features), window * 16, 1))
...
model = Sequential()
#model.add(layers.Embedding(input_length = (window * (14 + 2)) ))
#model.add(layers.core.SpatialDropout1D(0.2))
model.add(LSTM(100, input_shape = (window * 16, 1) ))
